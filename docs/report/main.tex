...
First, in \Cref{tab::bench_machine_100k} we give some benchmarks where all parties are run in individual threads on the same machine to establish a common baseline. These benchmarks are performed for a iris code size of $12800$ bits and a database size of $100\,000$ iris codes.

Next, in \Cref{tab::bench_network_100k} we perform the benchmarks for the same sizes on individual computing servers, where each party is connected to each other with a network speed of $6.6\,\text{Gbit}/s$, and a latency of $1.5\,ms$.

We continue by varying the database size and give benchmarks for $1\,000\,000$ iris codes in \Cref{tab::bench_network_1m}.


\begin{remark}
    We want to note, that we do not give benchmarks for the SPDZWise protocol in \Cref{tab::bench_machine_100k} and \Cref{tab::bench_network_1m} since the database unfortunately does not fit into the RAM which has only 64 GB.
\end{remark}

\begin{table}[ht]
    \centering
    \caption{Runtime and communication (in data sent per party) comparison of different MPC protocols, averaged over 5 runs. All parties run on the same machine, with each party having individual computing threads. Benchmarks for code size $|\texttt{iris}| = 12800$ bits and database of size $|\texttt{DB}| = 100\,000$ iris codes. Chunk Size refers to number of dot-products and MSB extraction operations batched per communication round.}
    \label{tab::bench_machine_100k}
...
\begin{table}[ht]
    \centering
    \caption{Runtime and communication (in data sent per party) comparison of different MPC protocols, averaged over 5 runs. All parties run on different machines in the same city (network speed $6.6\,\text{Gbit}/s$, latency $1.5\,ms$). Benchmarks for code size $|\texttt{iris}| = 12800$ bits and database of size $|\texttt{DB}| = 100\,000$ iris codes. Chunk Size refers to number of dot-products and MSB extraction operations batched per communication round.}
    ...
    \begin{tabular}{lrrcrr}
        ...
        \midrule
        Plain                        & 1                           & -                              & -                             & $0.165$                     & -                        \\
        ABY3                         & 1                           & 128                            & \xmark                        & $11.971$                    & $1.048$                  \\
        ABY3                         & 1                           & 1024                           & \xmark                        & $5.825$                     & $0.649$                  \\
        ABY3                         & 1                           & 10240                          & \xmark                        & $4.980$                     & $0.598$                  \\
        \midrule
        ABY3                         & 2                           & 10240                          & \xmark                        & $2.510$                     & $0.598$                  \\
        ABY3                         & 4                           & 10240                          & \xmark                        & $1.367$                     & $0.598$                  \\
        ABY3                         & 8                           & 10240                          & \xmark                        & $0.789$                     & $0.598$                  \\
        ABY3                         & 16                          & 10240                          & \xmark                        & $0.525$                     & $0.598$                  \\ ABY3                         & 16                          & 100096                          & \xmark                        & $0.438$                     & $0.598$                  \\
        ABY3                         & 32                          & 10240                          & \xmark                        & $0.475$                     & $0.598$                  \\
        ABY3                         & 32                          & 100096                         & \xmark                        & $0.361$                     & $0.598$                  \\
        \midrule
        SPDZWise                     & 1                           & 10240                          & \cmark                        & $7.351$                     & $4.957$                  \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[ht]
    \centering
    \caption{Runtime and communication (in data sent per party) comparison of different MPC protocols, averaged over 5 runs. All parties run on different machines in the same city (network speed $6.6\,\text{Gbit}/s$, latency $1.5\,ms$). Benchmarks for code size $|\texttt{iris}| = 12800$ bits and database of size $|\texttt{DB}| = 1\,000\,000$ iris codes. Chunk Size refers to number of dot-products and MSB extraction operations batched per communication round.}
    ...
    \begin{tabular}{lrrcrr}
        ...
        \midrule
        Plain                        & 1                           & -                              & -                             & $1.649$                     & -                        \\
        ABY3                         & 1                           & 10240                          & \xmark                        & $49.699$                    & $5.925$                  \\
        \midrule
        ABY3                         & 32                          & 10240                          & \xmark                        & $4.558$                     & $5.925$                  \\
        ABY3                         & 32                          & 100096                         & \xmark                        & $3.940$                     & $5.925$                  \\
        \bottomrule
    \end{tabular}
\end{table}

\todo[inline]{Add all benchmarks and discuss!}
