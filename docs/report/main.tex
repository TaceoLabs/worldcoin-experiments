...
Next, we continue by varying the database size and give benchmarks for $1\,000\,000$ iris codes in \Cref{tab::bench_network_1m}. Since we cannot benchmark SPDZWise for this database size due to RAM constraints, we benchmark this protocol for a smaller size of $10\,000$ in \Cref{tab::bench_network_10k} to get an idea of scaling.
...
Finally, we vary the network speed and give benchmarks for $1\,000\,000$ iris codes of size $12800$ bits in \Cref{tab::bench_network_100k_city} when executing the protocol when the servers are located in different cities. Concretely, one server ... \todo{here}

\begin{remark}
    We want to note, that we do not give benchmarks for the SPDZWise protocol in \Cref{tab::bench_network_1m} since the database unfortunately does not fit into the RAM which has only 64 GB.
\end{remark}
