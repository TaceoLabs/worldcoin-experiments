...
Due to its efficiency in checking correctness of AND gates, we recommend using it in combination with the joint message passing protocol (\Cref{sec::jmp}) to get malicious security.
...
As can be seen \Cref{tab::bench_network_100k_city}, network speed plays a huge role in the resulting performance. When sending smaller packages (i.e., small chunk sizes in the table) more communication rounds are required, which when having a slow latency, dominate the overall performance of the use case.


\section{Open Problems and Bottlenecks}

In this section, we discuss some of the bottlenecks in our implementation, give some ideas on how to improve them and touch on other areas that could be improved in future work.

\subsection{Size of the Shared Iris Database.}
...
\subsection{SPDZWise MACs and the effects on the Database.}
...
\subsection{Inputs are Bits shared over Larger Rings.}
...
\subsection{Dedicated SIMD implementation.}
