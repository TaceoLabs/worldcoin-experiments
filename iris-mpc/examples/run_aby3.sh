...
cargo build --release --example aby3
BIN=../target/release/examples/aby3

$BIN -p 0 -k data/key0.der -c examples/config.yaml -d data/db0.sqlite -i 0 --num-runs 1 $args &
$BIN -p 1 -k data/key1.der -c examples/config.yaml -d data/db0.sqlite -i 0 --num-runs 1 $args &
$BIN -p 2 -k data/key2.der -c examples/config.yaml -d data/db0.sqlite -i 0 --num-runs 1 $args
