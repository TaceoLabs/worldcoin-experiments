...
cargo build --profile profiling --example aby3
BIN=../target/profiling/examples/aby3

$BIN -p 0 -k data/key0.der -c examples/config.yaml -d data/db0.sqlite -i 0 $args &
